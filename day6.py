...
def set_lights(light_state, point_top, point_bottom):
  x1, y1 = get_xy(point_top)
  x2, y2 = get_xy(point_bottom)
  for x in range(x1, x2+1):
    for y in range(y1, y2+1):
      lights[x][y] = light_state

def set_light_bright(light_amount, point_top, point_bottom):
  x1, y1 = get_xy(point_top)
  x2, y2 = get_xy(point_bottom)
  for x in range(x1, x2+1):
    for y in range(y1, y2+1):
      lights[x][y] += light_amount
      if lights[x][y] < 0:
        lights[x][y] = 0

def toggle_lights(point_top, point_bottom):
  x1, y1 = get_xy(point_top)
  x2, y2 = get_xy(point_bottom)
  for x in range(x1, x2+1):
    for y in range(y1, y2+1):
...
for action in instructions:
  ...
  if action_info[0] == "toggle":
    #toggle_lights(action_info[1], action_info[3])
    set_light_bright(2, action_info[1], action_info[3])
  else:
    light_state = False
    light_amount = -1
    if action_info[1] == "on":
      light_state = True
      light_amount = 1
    #set_lights(light_state, action_info[2], action_info[4])
    set_light_bright(light_amount, action_info[2], action_info[4])
